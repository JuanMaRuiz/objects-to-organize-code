var myFeature = {
	...
	'init': function(config) {
		...
		// build the section-level nav and "click" the first item
		...
		// make a note that the initialization is complete; we don't strictly need this
	...
	'buildSectionNav': function($sections) {
		...
		$sections.each(function() {
		    ...
		    $('<li/>')
		      // use the text of the first h2 in the section as the text for the section navigation
		      .text($section.find('h2:first').text())

		      // add the list item to the section navigation
		      .appendTo(myFeature.$section_nav)

		      // use data() to store a reference to the original section on the newly-created list item
		      .data('section', $section)

		      // bind the click behavior to the newly created list item so it will show the section
	...
	'buildItemNav': function() {
		...
		// bind the click behavior to the newly created list item so it will show the content item
